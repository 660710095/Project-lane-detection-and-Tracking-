...
def get_perspective_matrices(img_shape):
    ...
    src = np.float32([
        ...
        [w * 0.35, int(h * 0.65)],    # 2. บนซ้าย: เล็งให้ตรงปลายเส้นประ
        [w * 0.55, int(h * 0.65)],    # 3. บนขวา: เล็งให้ตรงปลายเส้นทึบสีขาว
        [w * 0.72, h]            # 4. ล่างขวา: หุบเข้ามาเยอะๆ เพื่อเกาะแค่เส้นทึบ ทิ้งฟุตบาทไปเลย!
    ])
